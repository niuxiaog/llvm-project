...
//
//===----------------------------------------------------------------------===//
#include "mlir/Analysis/DataFlow/ConstantSubgraphAnalysis.h"
...
void ConstantSubgraphAnalysis::visitOperation(
  ...
  // if (operands.size() > 0) {
  //   bool all_operands_uninitialized = true;
  //   for (auto *operandLattice : operands) {
  //     if (!operandLattice->getValue().isUninitialized()) {
  //       all_operands_uninitialized = false;
  //       break;
  //     }
  //   }
  //   if (all_operands_uninitialized) {
  //     LLVM_DEBUG(llvm::dbgs() << "Curr op set to uninitialized\n");
  //     for (auto lattice : results) {
  //       propagateIfChanged(
  //           lattice, lattice->join(InConstantSubgraph::getUninitialized()));
  //     }
  //     return;
  //   }
  // }
  ...
  if (op->hasTrait<OpTrait::ConstantLike>()) {
    ...
    for (auto *operandLattice : operands) {
      ...
      // Operation *producer = operandLattice->getPoint().getDefiningOp();
      // if (producer) {
      //   if (auto emptyOp = dyn_cast<tensor::EmptyOp>(producer)) {
      //     // this operand is from an EmptyOp, treat is as constant
      //     operandState = true;
      //   }
      // }
      if (!operandState) {
...
void ConstantSubgraphAnalysis::setToEntryState(
  ...
  if (auto blockArg = lattice->getPoint().cast<BlockArgument>()) {
    ...
    std::optional<NamedAttribute> const_args =
        parent_op_attr.getNamed("linalg.const_args");
...
void RunConstantSubgraphAnalysis::getConstantSubgraph(DataFlowSolver &solver,
  ...
  for (Operation &op : llvm::make_early_inc_range(block)) {
    ...
    if (resultsAllConstant) {
      llvm::dbgs() << "ConstantOperation: " << op << "\n";
      op.setAttr("linalg.in_const_subgraph", builder.getBoolAttr(true));
  ...
  //     Operation *use = *(op.getUsers().begin());
  //     op.setAttr("linalg.in_const_subgraph",
  //                use->getAttr("linalg.in_const_subgraph"));
  //   }
  // }
  ...
  // constantOperations.front()->moveBefore(prevOp);
  // prevOp = constantOperations.front();
  // for (auto it_op = constantOperations.begin() + 1;
  //      it_op < constantOperations.end(); ++it_op) {
  //   (*it_op)->moveAfter(prevOp);
  //   prevOp = *it_op;
  // }
}
...
void RunConstantSubgraphAnalysis::run(Operation *topFunc) {
  if (func::FuncOp func = cast<func::FuncOp>(topFunc)) {
    llvm::dbgs() << "This is a func op\n";
  }
